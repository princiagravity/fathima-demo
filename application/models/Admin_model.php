...
class Admin_model extends CI_Model {
        ...
        {
        $sucess=1;        
        $query=$this->db->query("select id,username,name,email_id,role from user_details where username='$username' and password='$password' and role='admin'");
        $result=$query->row();
        if($result)
        {   
        $userdata=array(
        ...
        public function insert_offer_products($data=array())
        {
            $result= $this->db->insert('offer_product_details',$data);
            return $this->db->insert_id();
        }
        public function update_offer_products($data=array())
        {
            $this->db->where('id', $data['id']);
            $result= $this->db->update('offer_product_details',$data);
            if($this->db->affected_rows() >=0)
            return 1;
            else
            return 0;
        ...
        public function update_reward_point($data=array())
        {
            if($data['customer_id']=="all")
            {
                $result= $this->db->query("update user_add_details set reward_point=".$data['reward_point']." where role='customer' and status !='Deleted'"); 
            }
            else
            {
        ...
        {
            $query   = $this->db->query("SELECT id,name FROM category_master where status !='Deleted'");
            $results = $query->result();
            if($results)
       ...
       /*  public function get_addons($category='')
        ...
        public function get_product_name($product_id)
        {
            if($product_id)
            {
                $query   = $this->db->query("SELECT name from product_details where id=$product_id");
        ...
        {
            $query   = $this->db->query("SELECT id,customer_id,delivery_boy_id,area,cart_total,discount,invoice_no,order_time,tax,delivery_charge,delivery_tax,total_before_vat,tax_amount,order_total,payment_type,delivery_type,status,loc_latitude,loc_longitude,remarks,payment_status from order_details where id=$order_id");
    ...
    {
        $result=$this->db->query("select count(*) as order_count from order_details" )->row();
